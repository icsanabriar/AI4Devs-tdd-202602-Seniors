...
6. After staging, run `git diff --cached` and review it end-to-end before `git commit`. Re-check the secret rules in step 5. If the cached diff shows **large or unexpected binary additions** (or non-text formats not part of the intended change), stop and confirm with the user before committing.
